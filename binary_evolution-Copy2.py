...
#############new function##################    
    def mbin_cbd(self):
        ...
        no_cond = 0
        for mm in range(self.mtot.size):#tqdm(range(self.mtot.size)):
            ti = self.times[mm]
            mdoti = self.mdot[mm]
            if np.isnan(np.sum(R_vd[mm])):
                if np.isnan(np.sum(R_gw[mm])):
...
#                 else:
#                     condition = (self.scales[mm] > 0.0) & (self.scales[mm] < 1.0)# & (self.sep[mm]<=R_vd[mm][-1])   
#                 q_cbd[mm][q_cbd[mm]==-1] = np.full(q_cbd[mm][q_cbd[mm]==-1].shape, q_cbd[mm][q_cbd[mm]>0][-1]) 
#                 if len(np.where(condition)[0])<2:
#                     no_cond+=1                                                           
...
#                 idx = np.where(condition)[0]
#                 for ll in range(len(idx)-1):
#                     mbin_cbd[mm][idx[ll]+1] = mbin_cbd[mm][idx[ll]] + dmi[ll]
#                     m1_cbd[mm][idx[ll]+1], m2_cbd[mm][idx[ll]+1] = self.m1m2(mbin_cbd[mm][idx[ll]+1]
#                                                                              , q_cbd[mm][idx[ll]+1])
            else:
